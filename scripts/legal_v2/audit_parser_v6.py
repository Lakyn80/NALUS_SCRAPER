...
BASELINE_PATH = AUDIT_DIR / "v5_snapshot_baseline.json"


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Audit legal parser v6 against golden inputs and v5 snapshot output.")
...
def build_audit(review_dir: Path = DEFAULT_REVIEW_DIR) -> dict[str, Any]:
        ...
        docs_summary.append(
            {
                ...
                "v5_block_count": old["block_count"],
                "v6_block_count": document["block_count"],
    ...
    payload = {
        "schema_version": "parser-v6-audit.v1",
        "summary": summary,
...
def _snapshot_baseline(review_dir: Path) -> dict[str, Any]:
    ...
    return {"parser_profile": "legal-decision-parser.cz-courts.v5", "documents": documents}
...
def _changed_classes(old: dict[str, Any], new: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in new["lines"]:
        ...
        if old_row["class"] != row["class"]:
            rows.append({**_change_base(new, row), "v5_class": old_row["class"], "v6_class": row["class"], "reason": row["parser_state"]})
    return rows
            ...
            rows.append(
                {
                    ...
                    "v5_boundary": old_row["boundary"],
                    "v6_boundary": row["boundary"],
                    "block_impact": "split_changed" if row["boundary"] == "SPLIT" else "merge_changed",
                    "reason": "v6_structural_profile",
                }
            )
    return rows
            ...
            rows.append(
                {
                    ...
                    "source_id": new["source_id"],
                    "block_index": index,
                    "v5_range": old_at_index,
                    "v6_range": block_range,
                    "v5_classes": _classes_for_range(old, old_at_index),
                    "v6_classes": _classes_for_range(new, block_range),
                    "reason": "v6_block_restructure",
                }
            )
    return rows
...
def _summary(baseline: dict[str, Any], current: list[dict[str, Any]], golden_validation: dict[str, Any], changed_classes: list[dict[str, Any]], changed_boundaries: list[dict[str, Any]], changed_blocks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "parser_exceptions": 0,
        "v5_blocks": sum(doc["block_count"] for doc in baseline["documents"]),
        "v6_blocks": sum(doc["block_count"] for doc in current),
...
def _write_outputs(payload: dict[str, Any], golden_validation: dict[str, Any], changed_classes: list[dict[str, Any]], changed_boundaries: list[dict[str, Any]], changed_blocks: list[dict[str, Any]]) -> None:
    ...
    for court, name in (("constitutional_court", "constitutional_v5_vs_v6"), ("high_court_prague", "prague_v5_vs_v6"), ("high_court_olomouc", "olomouc_v5_vs_v6")):
    ...
    table = {"status": "pass", "table_row_changes": sum(1 for row in changed_classes if row.get("v6_class") == "list_or_table")}
