...
| Gold pass = keyword hit **and** source constraint in top-k | Yes (8 document-gold items) |
| Mixed corpus-only items | Keyword + `corpus_hit@k` only (no document gold) |

## ÚS — 5 document gold items
...
Remaining 8 mixed items stay `source_pending=true`.
